...
public class Xssf2Hssf {
    ...
    private void transform(XSSFWorkbook workbookOld, HSSFWorkbook workbookNew, XSSFRow rowOld, HSSFRow rowNew) {
        ...
        //log.info("rowOld size:{}",rowOld.getRowNum());

        HashMap<Integer, HSSFCellStyle> styleMap = new HashMap<Integer, HSSFCellStyle>();
        for (Cell cell : rowOld) {
            cellNew = rowNew.createCell(cell.getColumnIndex(), cell.getCellType());
            if (cellNew != null){
                this.transform(workbookOld, workbookNew, (XSSFCell) cell, cellNew, styleMap);
            }
        }
        this.lastColumn = Math.max(this.lastColumn, rowOld.getLastCellNum());
    }

    private void transform(XSSFWorkbook workbookOld, HSSFWorkbook workbookNew, XSSFCell cellOld, HSSFCell cellNew, HashMap<Integer, HSSFCellStyle> styleMap) {
        //FIXME excel预览功能转换为html，此处不支持富文本展示，屏蔽相关的批注转换
        //cellNew.setCellComment(cellOld.getCellComment());
        Integer hash = cellOld.getCellStyle().hashCode();
        //log.info("cellStyle hashCode:{},cellStyle:{},rowIndex:{}",hash,cellOld.getCellStyle(),cellOld.getRowIndex());
        if (styleMap != null && !styleMap.containsKey(hash)) {
            this.transform(workbookOld, workbookNew, hash, cellOld.getCellStyle(), (HSSFCellStyle) workbookNew.createCellStyle(), styleMap);
        }
        //log.info("styleMap size:{}",styleMap.size());
        //FIXME 样式种类超出4000种报错
    ...
    private void transform(XSSFWorkbook workbookOld, HSSFWorkbook workbookNew, Integer hash, XSSFCellStyle styleOld, HSSFCellStyle styleNew, HashMap<Integer, HSSFCellStyle> styleMap) {
